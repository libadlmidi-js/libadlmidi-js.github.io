...
// dist/libadlmidi.full.browser.js
async function createADLMIDI(moduleArg = {}) {
  ...
  function findWasmBinary() {
    if (Module["locateFile"]) {
      return locateFile("libadlmidi.full.browser.wasm");
    }
    return new URL("libadlmidi.full.browser.wasm", import.meta.url).href;
  }
...
var libadlmidi_full_browser_default = createADLMIDI;
...
var _AdlMidiProcessor = class _AdlMidiProcessor extends AudioWorkletProcessor {
  ...
  async initWasm(processorOptions) {
    try {
      ...
      const Module = await libadlmidi_full_browser_default(moduleConfig);
      this.adl = Module;
